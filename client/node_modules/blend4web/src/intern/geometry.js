...
function submesh_init_shape_keys(submesh, frame, shape_key_values) {
    ...
    // position
    var count = submesh.shape_keys.length;

    if (shape_key_values) {
        // init Basis
        var a_pos = submesh.shape_keys[0].geometry["a_position"];
    ...
    for (var i = 1; i < count; i++) {
        ...
        var value = shape_key_values ? shape_key_values[i].value : submesh.shape_keys[i].init_value;
    ...
    // tbn
    count = m_tbn.get_items_count(f_a_tbn);

    if (shape_key_values) {
        // init Basis
        var a_tbn = submesh.shape_keys[0].geometry["a_tbn"];
        for (var i = 0; i < count; i++) {
            m_tbn.set_item(f_a_tbn, a_tbn, i);
        }
    }
    ...
    for (var i = 0; i < count; i++) {
        ...
        for (var j = 1; j < submesh.shape_keys.length; j++) {
            ...
            var value = shape_key_values ? shape_key_values[j].value : submesh.shape_keys[j].init_value;
            if (!value)
                continue;
...
function update_bufs_data_array(bufs_data, attrib_name, num_comp, array) {
    ...
    if (pointer) {
        ...
        if (num_comp && pointer.num_comp != num_comp)
            m_assert.panic("invalid num_comp for \"" + attrib_name + "\"");
...
function extract_array_float(bufs_data, name) {
    if (name == "a_normal") {
        ...
        if (pointer) {
            ...
            m_assert.panic("extract_array_float() failed; invalid name: " + name);
    }
}
...
function update_draw_mode(bufs_data, draw_mode) {
    ...
    case exports.DM_DYNAMIC_TRIANGLES:
        ...
        break;
    default:
        m_assert.panic("Wrong draw_mode");
    }
...
function update_gl_buffers(bufs_data) {
    ...
    for (var i = 0; i < bufs_data.vbo_source_data.length; i++) {
        ...
        if (index == -1) {
            bufs_data.vbo_data.push({ vbo: null, type: type, debug_id: m_generator.unique_id() });
...
exports.submesh_apply_particle_transform = function (submesh, transform) {
    ...
    if (au_center_pos && au_center_pos.length) {
        ...
        m_assert.panic("Attribute \"au_center_pos\" is missing in particle submesh");
        ...
        for (var i = 0; i < va_frames.length; i++) {
            ...
            for (var j = 0; j < count; j++) {
                ...
                case "a_shade_tangs":
                    ...
                    break;
                default:
                    m_assert.panic("Wrong attribute name: " + param_name);
                    break;
                }
            }
        }
...
function num_comp(array, base_length) {
    ...
    if (factor != Math.floor(factor))
        m_assert.panic("Array size mismatch during geometry calculation: array length=" +
