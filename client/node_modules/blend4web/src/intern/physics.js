...
import m_debug_telemetry_fact from "./debug/telemetry.js";
...
import {get_build_version} from "./version.js";
...
function process_message(worker, msg_id, msg) {
    ...
    case m_ipc.IN_ERROR:
        ...
        break;
    case m_ipc.IN_FBMSG:
        m_debug_telemetry.fbmsg.apply(this, msg.slice(1));
        break;
...
function find_body_id_by_obj(obj) {
    for (var id in _bounding_objects) {
        if (_bounding_objects[id] === obj)
            return id;
    }
    return -1;
}
...
function prepare_static_mesh_physics(obj, batch, update) {
    if (update)
    ...
    return {
        ...
        collision_id_num: col_id_num(batch.collision_id),
        collision_margin: batch.collision_margin,
        collision_group: batch.collision_group,
        collision_mask: batch.collision_mask
    }
}

function init_static_mesh_physics(obj, batch, worker) {
    var r = prepare_static_mesh_physics(obj, batch);
    m_ipc.post_msg(worker, m_ipc.OUT_APPEND_STATIC_MESH_BODY, r.body_id, r.positions,
            r.indices, r.trans, r.friction, r.restitution, r.collision_id_num,
            r.collision_margin, r.collision_group, r.collision_mask);

    var phy = init_physics(r.body_id, "STATIC_MESH");
    phy.collision_id = r.collision_id;
    phy.collision_id_num = r.collision_id_num;
    return phy;
}

function update_static_mesh_physics(obj, batch, worker) {
    var r = prepare_static_mesh_physics(obj, batch, true);
    m_ipc.post_msg(worker, m_ipc.OUT_UPDATE_STATIC_MESH_BODY, r.body_id, r.positions,
            r.indices, r.trans, r.friction, r.restitution, r.collision_id_num,
            r.collision_margin, r.collision_group, r.collision_mask);
}
...
function init_bounding_physics(obj, compound_children, worker) {
    ...
    m_ipc.post_msg(worker, m_ipc.OUT_APPEND_BOUNDING_BODY, p.body_id, p.trans, p.quat,
        p.physics_type, p.is_ghost, p.disable_sleeping, p.mass,
        p.velocity_min, p.velocity_max, p.damping, p.rotation_damping,
        p.collision_id_num, p.collision_margin, p.collision_group, p.collision_mask,
        p.bounding_type, p.worker_bounding, p.size, p.friction,
        p.restitution, p.comp_children_params, p.correct_bound_offset);

    _bounding_objects[p.body_id] = obj;
    _bounding_objects_arr.push(obj);

    var phy = init_physics(p.body_id, "BOUNDING");
    phy.type = p.physics_type;
    phy.mass = p.mass;
    phy.is_ghost = p.is_ghost;
    phy.collision_id = p.collision_id;
    phy.collision_id_num = p.collision_id_num;
    return phy;
}

function update_bounding_physics(obj, compound_children, worker) {
    var p = prepare_bounding_physics_props(obj, compound_children, true);
    m_ipc.post_msg(worker, m_ipc.OUT_UPDATE_BOUNDING_BODY, p.body_id, p.trans, p.quat,
        p.physics_type, p.is_ghost, p.disable_sleeping, p.mass,
        p.velocity_min, p.velocity_max, p.damping, p.rotation_damping,
        p.collision_id_num, p.collision_margin, p.collision_group, p.collision_mask,
        p.bounding_type, p.worker_bounding, p.size, p.friction,
...
function init_character(obj, worker) {
    ...
    var rot_angle     = dir_ground_proj_angle(obj);
...
exports.enable_simulation = function(obj) {
    var phy = obj.physics;
    if (!phy)
        m_assert.panic("No object physics");
...
exports.sync_bounding = sync_bounding;
/**
 * Sync object bounding with engine.
 * @methodOf physics
 */
function sync_bounding(obj, scene) {
    var phy_set = obj.physics_settings;
    var worker = find_worker_by_scene(scene);
    var compound_children = [];
    ...
    if (is_vehicle_chassis(obj) || is_vehicle_hull(obj) ||
            obj.is_character || is_floater_main(obj) || obj.use_obj_physics) {
        if (!is_navigation_mesh(obj))
            update_bounding_physics(obj, compound_children, worker);
    }
}

exports.sync_mesh = sync_mesh;
/**
 * Sync object mesh with engine.
 * @methodOf physics
 */
function sync_mesh(obj, scene) {
    var phy_set = obj.physics_settings;
    var worker = find_worker_by_scene(scene);

    var sc_data = obj.scenes_data[0];
    var batches = sc_data.batches;
    ...
    for (var i = 0; i < batches.length; i++) {
        ...
        m_batch.update_batch_geometry(batches[i], batches[i].submesh, obj.render.shape_keys_values);

        if (batch.use_ghost)
            m_print.error("The function is not implemented for ghost objects yet");
        else
            update_static_mesh_physics(obj, batch, worker);

        recalc_collision_tests(scene, worker, obj.physics.collision_id);
    }
}
...
function collision_id_to_body_ids(collision_id, bpy_scene, dest_arr, dest_obj) {
    ...
    if (!dest_arr && !dest_obj)
        m_assert.panic("At least one destination required");
