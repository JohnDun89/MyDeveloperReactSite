...
import * as m_mat from "./material.js";
...
function update_object(bpy_obj, obj) {
    ...
    case "MESH":
        ...
        if (render.selectable) {
            // assign color id
            render.color_id = m_generator.gen_color_id(_color_id_counter);
            _color_id_counter++;
        }
...
function update_obj_pbr_reflections(obj) {
    ...
    for (var i = 0; i < materials.length; i++) {
        var mat = materials[i];
        if (mat.use_nodes) {
            if (check_bsdf_type(mat.node_tree, "BSDF_GLOSSY")
                    || check_bsdf_type(mat.node_tree, "BSDF_DIFFUSE")
                    || check_bsdf_type(mat.node_tree, "BSDF_PRINCIPLED")) {
...
function prepare_shape_keys(bpy_obj, obj) {
    if (bpy_obj["type"] != "MESH")
        m_assert.panic("Wrong object type: " + bpy_obj["name"]);
...
exports.copy = function(obj, name, deep_copy) {
    ...
    new_obj.render.color_id = m_generator.gen_color_id(_color_id_counter);
    _color_id_counter++;
...
function obj_switch_cleanup_flags(obj, cleanup_bufs, cleanup_shader, cleanup_nodemat) {
    for (var i = 0; i < obj.scenes_data.length; i++) {
        ...
        for (var j = 0; j < batches.length; j++) {
            ...
            m_batch.set_cleanup_nodemat(cleanup_nodemat);
        }
    }
}
...
exports.inherit_material = function(obj_from, mat_from_name, obj_to, mat_to_name) {
    ...
    for (var i = 0; i < obj_to.scenes_data.length; i++) {
        ...
        m_scenes.append_object(scene, obj_to, false);
        m_phy.append_object(obj_to, scene);
...
//  TODO: move next functions to internal material.js
exports.set_nodemat_value = function(obj, mat_name, node_name, value) {
    ...
    for (var i = 0; i < obj.scenes_data.length; i++) {
        ...
        for (var j = 0; j < batches.length; j++) {
            ...
            if (is_world || batch.material_names.indexOf(mat_name) != -1)
                is_changed |= m_batch.set_nodemat_value(batch, node_name, value);
        }
    }

    if (is_changed && is_world)
        m_scenes.update_sky_texture(obj);

    return is_changed;
}

exports.set_nodemat_rgb = function (obj, mat_name, node_name, r, g, b) {
    var is_world = m_obj_util.is_world(obj);
    ...
    for (var i = 0; i < obj.scenes_data.length; i++) {
        ...
        for (var j = 0; j < batches.length; j++) {
            var batch = batches[j];
            if (is_world || batch.material_names.indexOf(mat_name) != -1)
                is_changed |= m_batch.set_nodemat_rgb(batch, node_name, r, g, b);
        }
    }
...
exports.get_nodemat_value = function(obj, mat_name, node_name) {
    // TODO: check if there is allways MAIN batch
    var batch = m_batch.find_batch_material_any(obj, mat_name, "MAIN")
            || m_batch.find_batch_material_any(obj, mat_name, "PARTICLES");

    return m_batch.get_nodemat_value(batch, node_name);
}

 exports.get_nodemat_rgb = function(obj, mat_name, node_name, dest) {
    // TODO: check if there is allways MAIN batch
    var batch = m_batch.find_batch_material_any(obj, mat_name, "MAIN")
            || m_batch.find_batch_material_any(obj, mat_name, "PARTICLES");

    return m_batch.get_nodemat_rgb(batch, node_name, dest);
}
