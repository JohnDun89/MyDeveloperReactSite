...
exports.generate_main_batches = function(scene, bpy_mesh_objects, lamps,
    ...
    var lights = (lamps || []).map(function (lamp) { return lamp.light;});

    // create merged metabatches
    var metabatches = make_metabatches(bpy_mesh_objects, scene._render.graph, lights);
    ...
    for (var i = 0; i < metabatches.length; i++) {
        ...
        if (metabatches[i].render.type == "STATIC" && batch.type != "COLOR_ID") {
            ...
            var unique_name = m_generator.unique_name("%meta%" + batch.type + "%" +
...
function make_metabatches(bpy_mesh_objects, graph, lights) {
    var metabatches = [];
    ...
    for (var i = 0; i < bpy_mesh_objects.length; i++) {
        ...
        metabatches.push.apply(metabatches, make_object_metabatches(bpy_obj, render, graph, lights));
    }
 ...
 */
function make_object_metabatches(bpy_obj, render, graph, lights) {
    var metabatches = [];
    ...
    for (var i = 0; i < batch_types.length; i++) {
        ...
        for (var j = 0; j < materials.length; j++) {
            ...
            // FIXME: replace initialization.lights to better place
            batch.lights = m_util.clone_object_r(lights);
            ...
            if (material.is_lens_flares)
                submesh = prepare_lens_flares(submesh);
    ...
    if (psystems.length > 0) {
        ...
        var particles_metabatches = make_particles_metabatches(bpy_obj, render,
                graph, emitter_vc, em_submesh, lights);
...
function prepare_lens_flares(submesh) {
...
function make_particles_metabatches(bpy_obj, render, graph, emitter_vc,
    ...
    for (var i = 0; i < psystems.length; i++) {
        ...
        if (pset["type"] == "EMITTER") {
            ...
            var use_particles_rotation = m_util.version_cmp(cfg_def.loaded_data_version, REQUIRED_FOR_PART_SYS_BIN_FORMAT) != -1
            ...
            if (pset["render_type"] == "OBJECT") {
                ...
                var hair_metabatches = make_hair_particles_metabatches(
                        ...
                        [ptrans], pset, psys, use_grass_map, seed, false, lights);
                ...
                var hair_metabatches = make_hair_particles_metabatches(
                        ...
                        use_grass_map, seed, reset_seed, lights);
            ...
            m_assert.panic("Unknown particle settings type");
    }
...
function merge_metabatches(metabatches) {
    ...
    for (var i = 0; i < unique_data.length; i++) {
        ...
        if (submeshes.length == 0)
            var submesh = m_geom.init_submesh(m_generator.unique_name("%empty"));
...
function update_batch_material(batch, material, mesh) {
    ...
    case "PARTICLES":
        ...
        break;
    default:
        m_assert.panic("Wrong batch type: " + batch.type);
    }

    return ret;
}

function get_nodemat_id(mat_uuid, shader_type, active_layer_names) {
    var scene_uuid = "";
    if (m_scenes.check_active()) {
        var active_scene = m_scenes.get_active();
        scene_uuid = active_scene["uuid"];
    }
    return m_nodemat.generate_graph_id(mat_uuid, shader_type, scene_uuid, active_layer_names);
}
...
function update_batch_material_main(batch, material, mesh) {
    ...
    if (material.is_lens_flares) {
        ...
        if (!material.use_shadeless) {
            ...
            var id = get_nodemat_id(material.uuid, "MAIN");

            var nmat_graph = m_nodemat.create_lighting_graph(id, data, batch.lights);
...
function update_batch_blend_mode(batch, material) {
    ...
    case "OPAQUE":             // Opaque               don't sort  don't blend
        ...
        break;
    default:
        m_assert.panic("Unknown alpha blend mode: " + material.blend_mode);
    }
}
...
function init_water_material(material, batch) {
    ...
    var id = get_nodemat_id(material.uuid);
    var nmat_graph = m_nodemat.create_lighting_graph(id, data, batch.lights);
    batch.shaders_info.node_elements =
            m_nodemat.compose_node_elements(nmat_graph);
...
function update_batch_material_nodes(batch, material, mesh, shader_type) {
    ...
    var active_layer_names = {
        vc_name: mesh["active_vcol_name"],
        uv_name: mesh["active_uv_name"]
    };
    var id = get_nodemat_id(material.uuid, shader_type, active_layer_names);
    var ngraph_proxy = m_nodemat.compose_ngraph_proxy(node_tree, id,
            false, mesh["name"], material.name, shader_type,
            active_layer_names, batch.lights);
    ...
    m_graph.traverse(ngraph_proxy.graph, function(node, attr) {
        ...
        case "BSDF_TRANSPARENT":
            if (attr.data.value.use_alpha) {
                set_batch_directive(batch, "USE_BSDF_TRANSPARENT_ALPHA", 1);
            } else {
                batch.blend = true;
                batch.refractive = true;
                set_batch_directive(batch, "USE_BSDF_TRANSPARENT_REFRACT", 1);
            }
            break;
        case "BSDF_GLOSSY":
        case "BSDF_DIFFUSE":
...
exports.name_list_to_node_name = name_list_to_node_name;
function name_list_to_node_name(name_list, prefix_offset) {
    var id = name_list[prefix_offset];
    for (var i = prefix_offset + 1; i < name_list.length; i++)
        id = join_name(id, name_list[i]);
    return id;
}
...
function update_batch_render(batch, render) {
    ...
    if (render.is_skinning && render.vertex_anim)
        m_assert.panic("Skinning and vertex animation are mutually exlusive");
}
...
function update_batch_geometry(batch, submesh, shape_key_values) {
    if (batch.type == "PHYSICS") {
        if (batch.subtype == "NAVMESH") {
            ...
            if (submesh.shape_keys.length > 0)
                m_geom.submesh_init_shape_keys(submesh, submesh.va_frames[0], shape_key_values);
...
function update_batch_particles_emitter(batch, psystem, obj) {
    ...
    case "ZX":
        ...
        break;
    default:
        m_assert.panic("Wrong billboard align value");
        break;
    }
        ...
        for (var j = 0; j < ptrans.length; j+=data_len) {
            ...
            if (pset["b4w_initial_rand_rotation"]) {
                ...
                case "Z":
                    ...
                    break;
                default:
                    m_assert.panic("Unsupported random rotation type: "
...
function update_batch_id(batch) {
    ...
    var update_cb = function(batch) {
        ...
        batch.id = 0;
        batch.id = m_generator.calc_variable_id(batch, 0);
    }
...
function update_shader(batch) {
    if (!batch.shaders_info)
        m_assert.panic("No shaders info for batch " + batch.name);
...
function validate_batch(batch) {
    ...
    if (shaders_info.status === m_shaders.VALID) {
        ...
        for (var attr in attributes) {
            var p = pointers[attr];
            if (!p)
                m_assert.panic("missing data for \"" + attr + "\" attribute");
        }
    }
...
exports.create_postprocessing_batch = function(post_effect) {
    ...
    case "FLIP_CUBEMAP_COORDS":
        ...
        break;
    default:
        m_assert.panic("Wrong postprocessing effect: " + post_effect);
        break;
    }
...
exports.set_cleanup_nodemat = function(batch, cleanup_nodemat) {
    if (batch.ngraph_proxy_id !== "") {
        var ngraph_proxy = m_nodemat.get_ngraph_proxy_cached(batch.ngraph_proxy_id);
        if (ngraph_proxy)
...
function generate_odd_id(batch, render, odd_id_prop) {
    ...
    if (render.dynamic_geometry)
        return "_odd_id_" + m_generator.unique_id();
...
exports.cleanup = function() {
 ...
 exports.set_nodemat_value = function(batch, node_name, value) {
    var inds = batch.node_value_inds;
    for (var i = 0; i < inds.length; i += 3) {
        if (inds[i] == node_name) {
            batch.node_values[inds[i + 1] * 4 + inds[i + 2]] = value;
            return true;
        }
    }
    return false;
...
exports.set_nodemat_rgb = function(batch, node_name, r, g, b) {
    var inds = batch.node_rgb_inds;
...
exports.get_nodemat_value = function(batch, node_name) {
    var inds = batch.node_value_inds;
    for (var i = 0; i < inds.length; i += 3) {
        if (inds[i] == node_name) {
            return batch.node_values[inds[i + 1] * 4 + inds[i + 2]];
        }
    }
    return null;
}

exports.get_nodemat_rgb = function(batch, node_name, dest) {
    var inds = batch.node_rgb_inds;
    for (var i = 0; i < inds.length; i += 2) {
        if (inds[i] == node_name) {
            var ind = inds[i + 1];
