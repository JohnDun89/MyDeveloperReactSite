...
function get_compiled_shader(shaders_info) {
    ...
    if (shaders_info.status === VALID) {
        ...
        if (f_compile_fail)
            report_shader_compiling_error(compiled_shader.fshader,
                    shader_id, fshader_text);
        else if (v_compile_fail)
            report_shader_compiling_error(compiled_shader.vshader,
                    shader_id, vshader_text);
        else if (link_fail)
            report_shader_linking_error(compiled_shader.program,
 ...
 */
function report_shader_compiling_error(shader, shader_id, shader_text) {
 ...
 */
function report_shader_linking_error(program, shader_id, vshader_text, fshader_text) {
...
function get_shader_ast(dir, filename) {
    ...
    if (!_shader_texts) {
        var ast = b4w.require("shader_texts")[filename];
        if (!ast)
            return null;
    } else {
        ...
        if (!main_text)
            return null;
        var ast = m_gpp_parser.parser.parse(main_text);
    }
...
exports.load_shaders = function() {
    ...
    if (!b4w.module_check("shader_texts")) {
        ...
        for (var i = 0; i < SHADERS.length; i++) {
            var shader_path = normpath_preserve_protocol(cfg_pth.shaders_path
                    + SHADERS[i]);
...
function preprocess_shader(type, ast, shaders_info) {
    ...
    function process_group(elem) {
        ...
        for (var i = 0; i < parts.length; i++) {
            ...
            case "txt":
                ...
                break;
            default:
                m_assert.panic("Unknown element type: " + pelem.TYPE);
                break;
            }
        }
    }
    ...
    function eval_expression(expr_list) {
        ...
        for (var i = 0; i < expr_list.length; i++) {
            if (!(expr_list[i] instanceof Object)) {
                ...
                case "logic_negative_expr":
                    ...
                    break;
                default:
                    m_assert.panic("Unknown operation type: " + expr_list[i].TYPE);
                    break;
                }
            }
        }
        if (operand_stack.length == 1)
            ...
            m_assert.panic("Incorrect expression: " + expr_list.join(" "));
    }
    ...
    function process_error(elem) {
        var tokens = elem.TOKENS;
        m_assert.panic("Shader error: #error " + tokens.join(" "));
    }
...
function glsl_value(value, dim) {
    ...
    case 16:
        ...
        break;
    default:
        m_assert.panic("Wrong glsl value dimension");
        break;
    }
}
