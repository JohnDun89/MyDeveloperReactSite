...
import * as m_md5 from "../libs/md5.js";
...
var FORCING_BSDF_TYPES = ["BSDF_PRINCIPLED", "BSDF_GLOSSY", "BSDF_DIFFUSE", "BSDF_TRANSPARENT"];
...
function load_main(bpy_data, thread, stage, cb_param, cb_finish,
    ...
    if (!main_path)
        m_assert.panic("Nothing requested");
...
function load_binaries(bpy_data, thread, stage, cb_param, cb_finish,
    ...
    if (!binary_path)
        m_assert.panic("Binary data is missing");
...
function check_format_version(loaded_bpy_data) {
    ...
    case -1:
        if (ver_loaded[0] < cfg_def.min_format_version[0])
            m_assert.panic("JSON version is too old relative to B4W engine: "
    ...
    case 1:
        if (ver_loaded[0] > cfg_def.min_format_version[0])
            m_assert.panic("B4W engine version is too old relative to JSON. "
...
function check_bin_data_version(bin_data, bpy_data) {
    ...
    if (headers[0] != ver_loaded[0])
        m_assert.panic("BIN version does not match to JSON version: " 
...
function duplicate_objects_iter(obj_links, origin_obj, obj_ids, grp_ids, cluster_data) {
    ...
    for (var i = 0; i < obj_links.length; i++) {
        ...
        if (bpy_obj["dupli_group"]) {
            ...
            grp_new["name"] = m_generator.unique_name(grp["name"]+"_CLONE");
...
function make_bpy_links(bpy_data) {
    ...
    for (var i = 0; i < objects.length; i++) {
        ...
        case "MESH":
            ...
            if (!bpy_obj["data"])
                m_assert.panic("mesh not found for object " + bpy_obj["name"]);
...
function load_images(bpy_data, thread, stage, cb_param, cb_finish, cb_set_rate) {
    ...
    for (var i = 0; i < images.length; i++) {
        ...
        if (uuid) {
            var image_path = normpath_preserve_protocol(dir_path + image["filepath"]);
    ...
    for (var i = 0; i < objects.length; i++) {
        ...
        if (vh_set_i.part == "CHASSIS") {
            ...
            if (obj_i.vehicle.props.length != 4)
                m_assert.panic("Not enough wheels for chassis " + obj_i.name);
