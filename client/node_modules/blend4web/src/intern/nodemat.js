...
import m_debug_nodegraph_fact from "./debug/nodegraph.js";
...
var m_debug_nodegraph = m_debug_nodegraph_fact(ns);
...
function compose_ngraph_proxy(node_tree, ntree_graph_id, is_node_group,
        mat_user_name, mat_name, shader_type, active_layer_names, lights) {

    var ngraph_proxy = compose_ngraph_proxy_rec(node_tree, ntree_graph_id, 
        is_node_group, shader_type, active_layer_names, lights);
...
function compose_ngraph_proxy_rec(node_tree, ntree_graph_id, is_node_group, 
        shader_type, active_layer_names, lights) {
    ...
    var ngraph_proxy = {
        graph: null,
        id: ntree_graph_id,
        // TODO: check cleanup, probably there is a memory leak.
    ...
    if (shader_type != "SHADOW" && shader_type != "COLOR_ID") {
        ...
        for (var i = 0; i < bpy_nodes.length; i++) {
            ...
            if (append_nmat_node(graph, bpy_node, 0, shader_type, 
                    active_layer_names, lights) == null) {
        ...
        if (!is_world_mat) {
            split_material_nodes(graph_out, shader_type, lights);
            split_cycles_output_nodes(graph_out, shader_type, lights);
        } else {
        ...
        var main_ngraph_proxy = compose_ngraph_proxy_rec(node_tree, ntree_graph_id, 
                is_node_group, "MAIN", active_layer_names, lights);
...
exports.create_lighting_graph = function (ntree_graph_id, data, lights) {
    ...
    var bpy_node = {"name": "LIGHTING_BEGIN",
                    "type": "LIGHTING_BEGIN"};
    var begin_node_id = append_nmat_node(graph, bpy_node, 0, null, null, lights);

    bpy_node = {"name": "LIGHTING_END",
                "type": "LIGHTING_END"};
    var end_node_id = append_nmat_node(graph, bpy_node, 0, null, null, lights);
    var translucency_edges = [[begin_node_id, [8, 10]],
                             [begin_node_id, [9, 6]]];
    add_lighting_subgraph(graph, data, begin_node_id, end_node_id, 
            translucency_edges, lights);
    clean_sockets_linked_property(graph);
    _composed_stack_graphs[ntree_graph_id] = graph;
    return graph;
}

function split_material_nodes(graph, shader_type, lights) {
    var material_nodes = [];
    ...
    for (var i = 0; i < material_nodes.length; ++i) {
        ...
        add_lighting_subgraph(graph, node.data.value, material_begin_id, 
                material_end_id, translucency_edges, lights);
...
function split_cycles_output_nodes(graph, shader_type, lights) {
    ...
    for (var i = 0; i < output_material_nodes.length; ++i) {
        ...
        if (use_surface) {
            ...
            split_surface_output_nodes(graph, shader_type, lights);
...
function split_surface_output_nodes(graph, shader_type, lights) {
    ...
    for (var i = 0; i < output_surface_nodes.length; ++i) {
        ...
        for (var k = 0; k < in_count; k++) {
            ...
            if (in_node.type == "BSDF_GLOSSY" || in_node.type == "BSDF_DIFFUSE" || in_node.type == "MIX_SHADER" ||
                    in_node.type == "EMISSION" || in_node.type == "BSDF_TRANSPARENT" || in_node.type == "BSDF_PRINCIPLED") {
                // d_color
        ...
        add_bsdf_subgraph(graph, node.data.value, bsdf_begin_id, bsdf_end_id, translucency_edges, lights);

        append_edges_in.push([bsdf_end_id, node_id, [0,0]]);
        append_edges_in.push([bsdf_end_id, node_id, [1,1]]);
...
function add_bsdf_subgraph(graph, data, begin_node_id, end_node_id, translucency_edges, lights) {
    var bpy_node = {"name": "LIGHTING_AMBIENT",
                    "type": "LIGHTING_AMBIENT"};
    var curr_node_id = append_nmat_node(graph, bpy_node, 0, null, null, []);
    var prev_node_id = curr_node_id;
...
exports.generate_graph_id = function(graph_id, shader_type, scene_id, active_layer_names) {
...
function append_nmat_node(graph, bpy_node, output_num, shader_type, 
    ...
    case "ATTRIBUTE":
        ...
        type = bpy_node_vc_layer_name ? "GEOMETRY_VC" : "EMPTY_VC";
    ...
    case "BSDF_TRANSPARENT":
        var use_alpha = bpy_node["b4w_use_alpha"];
        ...
        outputs = [bsdf_output,
                   ...
                   default_node_inout("emisson", "emisson", 0, bsdf_output_is_linked),
                   default_node_inout("a_color", "a_color", [0, 0, 0], bsdf_output_is_linked),
                   default_node_inout("alpha", "alpha", use_alpha ? 0 : 1, bsdf_output_is_linked)];
        ...
        dirs.push(["USE_ALPHA", use_alpha | 0]);
        break;
    case "EMISSION":
        inputs = node_inputs_bpy_to_b4w(bpy_node);
    ...
    case "OUTPUT_MATERIAL":
        ...
        var bsdf_end_outputs = [default_node_inout("color", "color", [0, 0, 0], surface_inp_is_linked),
                                default_node_inout("alpha", "alpha", 1, surface_inp_is_linked)];
...
function process_node_group(bpy_node, shader_type, active_layer_names, lights) {
    ...
    var ngraph_proxy_group = compose_ngraph_proxy_rec(node_tree,
            bpy_node["node_group"]["uuid"], true, shader_type,
            active_layer_names, lights);
    var data = {
...
function texture_node_type(bpy_node) {
    ...
    for (var i = 0; i < outputs.length; i++) {
        ...
        case "Value":
            ...
            break;
        default:
            m_assert.panic("Unknown texture output");
        }
    }
...
function print_node_graph(node_graph, mat_user_name, mat_name) {
    m_print.log("\n====== USER: " + mat_user_name + ", MATERIAL: " 
            + mat_name + " ======" 
            + "\n" + m_debug_nodegraph.nodegraph_to_dot(node_graph, true)
            + "\n=============================================================");
}
