...
import m_debug_render_time_fact from "./debug/render_time.js";
...
exports.draw = function(subscene) {
    ...
    if (subscene.type == m_subs.RESOLVE) {
        m_debug_render_time.start_subs(subscene);
        draw_resolve(subscene);
        m_debug_render_time.stop_subs(subscene);

        m_debug_check.gl("draw resolve");
    } else if (subscene.type == m_subs.COPY) {
        m_debug_render_time.start_subs(subscene);
        draw_copy(subscene);
        m_debug_render_time.stop_subs(subscene);

        m_debug_check.gl("draw copy");
    } else {
        m_debug_render_time.start_subs(subscene);
        ...
        m_debug_check.gl("draw subscene: " + m_subs.subs_label(subscene));
        // NOTE: fix for strange issue with skydome rendering
        // NOTE: commented below code was checked on Windows. All is fine.
        // UPD: BRDF subs framebuffer overridden (Santa 2017 scene with webplayer)
        if (subscene.type == m_subs.BRDF)
    ...
    for (var i = 0; i < draw_data.length; i ++) {
        ...
        for (var j = 0; j < bundles.length; j++) {
            ...
            if (bundle.do_render) {
                ...
                m_debug_render_time.start_batch(batch);
                if (m_debug_render_time.is_debug_view_render_time_mode() && batch.type == "DEBUG_VIEW")
                ...
                m_debug_render_time.stop_batch(batch);
                if (m_debug_render_time.is_debug_view_render_time_mode() && batch.type == "MAIN")
...
exports.clear = function(subscene) {
    if (subscene) {
        ...
        // Probably this branch is possible only in case of canvas clearing.
        var bc = cfg_def.background_color;
        if (bc[0] === BLACK_BG_COLOR[0] && bc[1] === BLACK_BG_COLOR[1] &&
                bc[2] === BLACK_BG_COLOR[2] && bc[3] === BLACK_BG_COLOR[3])
            return;
    }
    clear_binded_framebuffer(subscene);
}
...
function draw_sky(subscene, batch, shader) {
    ...
    // CHECK: scenes.update clears current framebuffer, so we should reattach it.
    _gl.bindFramebuffer(_gl.FRAMEBUFFER, null);
}
