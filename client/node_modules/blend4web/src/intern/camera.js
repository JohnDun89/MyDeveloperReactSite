...
function move_style_bpy_to_b4w(bpy_move_style) {
    ...
    case "HOVER":
        ...
        break;
    default:
        m_assert.panic("Unknown move style");
    }
}
...
function create_camera(type) {
    ...
    case exports.TYPE_HMD_RIGHT:
        m_assert.panic("Stereo camera may only be created from perspective one");
        break;
    default:
        m_assert.panic("Unknown camera type");
        break;
    }
...
exports.check_attachment = function(cam, type) {
    ...
    default:
        m_assert.panic("Wrong attachment type: " + type);
        break;
    }
}
...
exports.make_stereo = function(cam, type) {
    ...
    if (!(cam.type == exports.TYPE_PERSP &&
        ...
        m_assert.panic("make_stereo(): wrong camera type");
...
function set_stereo_params(cam, conv_dist, eye_dist) {
    ...
    if (!(cam.type == exports.TYPE_STEREO_LEFT ||
        ...
        m_assert.panic("set_stereo_params(): wrong camera type");
...
function get_camera_angles(cam, dest) {
    ...
    return get_camera_angles_from_quat(quat, cam.render.vertical_axis, dest);
}
...
function get_camera_angles_from_quat(quat, vertical_axis, dest) {
    var horizontal_axis_my = _vec3_tmp;
    var horizontal_axis_x = _vec3_tmp2;

    if (m_vec3.dot(vertical_axis, m_util.AXIS_Z) > 0) {
        m_vec3.cross(m_util.AXIS_X, vertical_axis, horizontal_axis_my);
        m_vec3.normalize(horizontal_axis_my, horizontal_axis_my);
    } else {
        m_vec3.copy(m_util.AXIS_Z, horizontal_axis_my);
    }
    m_vec3.cross(vertical_axis, horizontal_axis_my, horizontal_axis_x);
    ...
    var proj_z = m_vec3.dot(my_world_cam, vertical_axis);
    if (Math.abs(base_theta) > Math.PI / 4) {
        var phi_dir = m_vec3.scale(my_world_cam, -m_util.sign(base_theta), _vec3_tmp5);
    } else {
        var phi_dir = m_vec3.scale(z_world_cam, -m_util.sign(Math.asin(proj_z)), _vec3_tmp6);
    }
    var base_phi = Math.atan(Math.abs(
            m_vec3.dot(horizontal_axis_x, phi_dir) /
            m_vec3.dot(horizontal_axis_my, phi_dir)));
    ...
    // resulted phi
    var phi = base_phi;
    if (m_vec3.dot(horizontal_axis_my, phi_dir) < 0)
        phi = Math.PI - phi;
    if (m_vec3.dot(horizontal_axis_x, phi_dir) < 0)
        phi = 2 * Math.PI - phi;
...
function set_frustum(cam, fov, near, far) {
    ...
    case exports.TYPE_HMD_RIGHT:
        break;
    default:
        m_assert.panic("set_frustum(): Unsupported camera type: " + cam.type);
        break;
    }
...
exports.set_frustum2 = function(cam, h_fov_or_right, v_fov_or_top, near, far) {
    ...
    case exports.TYPE_HMD_RIGHT:
        break;
    default:
        m_assert.panic("set_frustum2(): Unsupported camera type: " + cam.type);
        break;
    }
...
function set_frustum_asymmetric(cam, left, right, bottom, top, near, far) {
    ...
    case exports.TYPE_ORTHO_ASYMMETRIC:
        ...
        break;
    default:
        m_assert.panic("set_frustum_asymmetric(): " +
...
function is_upside_down(camobj) {
    ...
    var vertical_axis = camobj.render.vertical_axis;
    var y_world_cam = m_util.quat_to_dir(quat, m_util.AXIS_Y, _vec3_tmp);
    return m_vec3.dot(y_world_cam, vertical_axis) < 0;
}
...
function set_rotation_static_angles(obj, phi, theta) {
    ...
    var render = obj.render;
    var quat = m_tsr.get_quat(render.world_tsr, _quat4_tmp);
    var curr_angles = get_camera_angles_from_quat(quat, render.vertical_axis, _vec2_tmp);
...
function camera_rotate_point_pivot(obj, pivot, d_phi, d_theta) {
    ...
    if (d_phi || d_theta) {
        ...
        if (d_phi) {
            m_quat.setAxisAngle(obj.render.vertical_axis, d_phi, _quat4_tmp);
        }
...
function set_projection(cam, ignore_after_actions) {
    ...
    case exports.TYPE_NONE:
        return;
    default:
        m_assert.panic("Wrong camera type: " + cam.type);
    }
...
function extract_frustum_corners(cam, near, far, corners, is_world_space) {
    ...
    case exports.TYPE_NONE:
        m_assert.panic("Extraction from NONE camera is not possible");
        break;
